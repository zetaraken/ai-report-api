...
app = FastAPI(title="AI매출업 리포트 API", version="1.0.0")
...
def create_token(email: str) -> str:
    ...
    return jwt.encode({"sub": email, "role": "admin", "exp": expire}, JWT_SECRET_KEY, algorithm=JWT_ALGORITHM)
...
MERCHANTS = [
    {"id":"m_test_001","name":"배포차","region":"서울 강남구","naver_place_url":"https://naver.me/xv6tlDW3","blog_keywords":"배포차, 신사역 배포차","instagram_hashtags":"배포차,신사역배포차","youtube_keywords":"배포차, 신사역 배포차"},
    {"id":"m_test_002","name":"온빈 신정호","region":"충남 아산시","naver_place_url":"https://naver.me/xdjavbzA","blog_keywords":"온빈 신정호, 신정호 맛집","instagram_hashtags":"온빈신정호,신정호맛집","youtube_keywords":"온빈 신정호, 신정호 한식"},
]
...
def make_sample_report(merchant: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"merchant_name":merchant["name"],"region":merchant.get("region",""),"generated_at":now_text(),"summary":summary,"monthly_summary":monthly,"channel_share":[{"name":"네이버 블로그","value":summary["naver_blog_count"]},{"name":"인스타그램","value":summary["instagram_count"]},{"name":"영수증 리뷰","value":summary["place_receipt_count"]},{"name":"플레이스 블로그 리뷰","value":summary["place_blog_count"]},{"name":"유튜브","value":sum(r["youtube_count"] for r in monthly)}],"top_videos":top_videos,"insights":["최근 게시물 수가 꾸준히 증가하고 있습니다.","네이버 블로그와 인스타그램에서 가맹점 언급이 확인됩니다.","네이버 플레이스 리뷰를 통해 실제 방문 반응도 확인됩니다.","유튜브 콘텐츠는 조회수 기반 인지도 확산에 일부 기여하고 있습니다."]}
...
def login(payload: LoginRequest):
    ...
    return {"access_token":create_token(payload.email),"token_type":"bearer","expires_in_minutes":JWT_EXPIRE_MINUTES,"admin_email":payload.email}

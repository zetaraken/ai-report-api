...
app = FastAPI(title="SNS 분석 솔루션 API", version="33.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# ════════════════════════════════════════════════════════════
# STEP 1: 영수증(방문자) 리뷰  v30
# 변경 사항 (v29 → v30):
#   ① 페이지 리로드 전략 제거
#      - 리로드하면 네이버가 1번 리뷰부터 다시 로드 → seen_texts 충돌 → +0건 무한반복
#   ② DOM 완전 제거 주기 단축: 8라운드 → 5라운드
#   ③ DOM 제거 대상 확대: li 뿐만 아니라 div.pui__vn15t2 도 remove()
#   ④ +0건 연속 카운트 도입:
#      - 3회 연속 +0건이면 추가 스크롤(6회) 후 재시도
#      - 5회 연속 +0건이면 완전 종료 (버튼 클릭돼도 더 이상 새 리뷰 없음)
#   ⑤ 버튼 클릭: JS evaluate 방식 유지 (v29에서 hang 문제 해결됨)
# ════════════════════════════════════════════════════════════
def crawl_receipt_reviews(place_id, target=500, progress_cb=None):
    ...
    # ③ 수집 완료 노드 DOM에서 완전 제거 (li + div 모두)
    JS_REMOVE_COLLECTED = """
        () => {
            let cnt = 0;
            // li 노드 제거
            ...
            // div.pui__vn15t2 제거 (li 제거 후 남은 경우)
    ...
    for url_type, attempt_url in urls_to_try:
        ...
        try:
            with sync_playwright() as p:
                ...
                round_num        = 0
                MAX_ROUNDS       = 120
                CLEANUP_INTERVAL = 5    # 5라운드마다 DOM 완전 제거
...
# ════════════════════════════════════════════════════════════
@app.get("/")
async def root(): return {"message":"SNS 분석 솔루션 API v33"}
